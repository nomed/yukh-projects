...
const packageDocument=JSON.parse(await readFile("package.json","utf8"));const changelog=await readFile("CHANGELOG.md","utf8");const paths=["RELEASE_NOTES.md","action.yml","apply-action.yml","dist/action/index.js","dist/apply/action.js","dist/apply/cli.js","dist/apply/index.js","dist/apply/manifest.json","dist/cli/index.js","dist/mcp-effect-b/index.js","dist/mcp-effect-b/manifest.json","test/fixtures/mcp-effect-b-bridge-v2-vector.json","package-lock.json","release-package.tgz","release-provenance.json","release.spdx.json"];const artifacts=await Promise.all(paths.map(async path=>({path,bytes:await readFile(path)})));const manifest=createWouldPublishManifest(version,commit,packageDocument.version,changelog,artifacts);process.stdout.write(`${JSON.stringify(manifest)}\n`);
