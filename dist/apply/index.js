...
function parseApplyCliArgs(argv) {
  ...
  if (!DIGEST.test(values.get("--approved-plan-id") ?? "") || !bounded(values.get("--environment") ?? "", 64)) throw new TypeError("invalid apply arguments");
...
function planReconciliation(input3) {
  ...
  for (const mapping of FIELD_MAP) {
    const raw2 = mapping.source(input3.contract);
...
var RestSnapshotClient = class {
  ...
  invalidate(input3, effect) {
    const projectOwnerLogin = input3.projectOwnerLogin ?? input3.ownerLogin;
    if (!/^[A-Za-z0-9-]{1,39}$/u.test(projectOwnerLogin) || !Number.isSafeInteger(input3.projectNumber) || input3.projectNumber < 1) throw new GitHubTransportError("YKP-GH-READ-001");
    const prefix = new RegExp(`^/(?:orgs|users)/${projectOwnerLogin}/projectsV2/${input3.projectNumber}/(?:${effect === "schema" ? "fields|items" : "items"})\\?`, `u`), keys2 = /* @__PURE__ */ new Set([...this.cache.keys(), ...this.flights.keys()]);
...
async function readWithClient(input3, options, client) {
  ...
  if (projectOwnerLogin === input3.ownerLogin) projectOwnerKind = repositoryOwnerKind;
  else {
  ...
  const projectPage = await client.get(`/${projectOwnerKind}/${projectOwnerLogin}/projectsV2/${input3.projectNumber}`), project = projectPage.body;
  ...
  const fieldsPage = await client.list(`/${projectOwnerKind}/${projectOwnerLogin}/projectsV2/${input3.projectNumber}/fields?per_page=100`);
  ...
  const itemsPage = await client.list(`/${projectOwnerKind}/${projectOwnerLogin}/projectsV2/${input3.projectNumber}/items?per_page=100${fieldSelector ? `&fields=${fieldSelector}` : ""}`);
  ...
  const issueTypes = options.includeIssueTypes && repositoryOwnerKind === "orgs" ? (await client.list(`/repos/${input3.ownerLogin}/${input3.repositoryName}/issue-types?per_page=100`)).nodes.map((value2) => ({ id: text3(value2.node_id), name: text3(value2.name, 128) })).sort((a, b) => a.id.localeCompare(b.id)) : void 0;
  return { subjectRef: subject(options.token), ownerKind: projectOwnerKind, ownerLogin: input3.ownerLogin, repositoryOwnerKind, projectOwnerKind, projectOwnerLogin, repositoryName: input3.repositoryName, projectNumber: input3.projectNumber, repositoryRef: text3(repo.node_id), projectRef, fields: fields.sort((a, b) => a.id.localeCompare(b.id)), ...issueTypes ? { issueTypes } : {}, issues, evidence: { ...client.evidence } };
}
...
function createGovernedHandoffReceipt(input3) {
  return parseDeferredReceiptV1({ schema: 1, version: "deferred-receipt-v1", status: "deferred", reason: input3.resource === "rest" ? "rest-reserve" : "graphql-reserve", issued_at_ms: input3.issuedAtMs, resume_after_ms: input3.resumeAfterMs ?? input3.issuedAtMs + 6e4, resume_by_ms: input3.resumeByMs ?? input3.issuedAtMs + 15 * 6e4, bindings: { scope_digest: input3.scopeDigest, request_digest: input3.requestDigest, plan_digest: input3.planDigest }, ownership: { disposition: "handoff", mode: "governed-handoff", wakeup_digest: null, cancellation_digest: null }, fresh_approval_required: input3.freshApprovalRequired });
}
...
function selectWorkTypeProvider(input3) {
  if (!input3 || !["users", "orgs"].includes(input3.projectOwnerKind) || !["users", "orgs"].includes(input3.repositoryOwnerKind) || !safe(input3.desired) || !safe(input3.fieldName) || !Array.isArray(input3.fields)) throw new WorkTypeProviderError("YKP-WORKTYPE-001");
  const projectValue = typeof input3.projectValue === "string" ? input3.projectValue : void 0;
  if (input3.nativeValue !== void 0 && projectValue !== void 0 && input3.nativeValue !== projectValue) throw new WorkTypeProviderError("YKP-WORKTYPE-003");
  if (input3.repositoryOwnerKind === "orgs") {
    const matches = (input3.issueTypes ?? []).filter((value2) => value2.name === input3.desired);
    if (matches.length !== 1 || !safe(matches[0]?.id, 256)) throw new WorkTypeProviderError("YKP-WORKTYPE-002");
    return { provider: "native_issue_type", desired: input3.desired, issueTypeId: matches[0].id, converged: input3.nativeValue === input3.desired };
  }
  const fields = input3.fields.filter((value2) => value2.name === input3.fieldName);
  if (fields.length > 1 || fields[0] && fields[0].kind !== "single_select") throw new WorkTypeProviderError("YKP-WORKTYPE-002");
  const field2 = fields[0], options = field2?.options.filter((value2) => value2.name === input3.desired) ?? [];
  if (field2 && options.length !== 1) throw new WorkTypeProviderError("YKP-WORKTYPE-002");
  return { provider: "project_work_type", desired: input3.desired, ...field2 ? { field: field2, optionId: options[0].id } : {}, converged: projectValue === input3.desired };
}
  ...
  return { create: async (input3) => {
    ...
    const variables2 = async (operation) => {
      ...
      if (operation.type === "set_issue_type") {
        ...
        return { kind: "set_issue_type", ownerLogin: snapshot.ownerLogin, repositoryName: snapshot.repositoryName, issueNumber: input3.requestedScope.issueNumber, issueTypeName: operation.desired };
      }
...
async function variables(operation, prepared, reader, input3) {
  ...
  if (operation.type === "set_issue_type") {
    if (typeof operation.desired !== "string") throw new ApplyPortError("invariant");
    return { kind: "set_issue_type", ownerLogin: snapshot.ownerLogin, repositoryName: snapshot.repositoryName, issueNumber: input3.issueNumbers[0], issueTypeName: operation.desired };
  }
  if (operation.type === "set_parent") {
...
// src/resumable-deferral-host.ts
function createResumableDeferralHost(port, nowMs = Date.now) {
  return {
    retain: async (source) => {
      const handoff = parseDeferredReceiptV1(source);
      if (handoff.ownership.disposition !== "handoff" || nowMs() > handoff.resume_by_ms) throw new TypeError("invalid deferral transition");
      const handles = await port.schedule(handoff), receipt = parseDeferredReceiptV1({ ...handoff, ownership: { disposition: "retained", mode: "durable-host", wakeup_digest: handles.wakeupDigest, cancellation_digest: handles.cancellationDigest } });
      return receipt;
    },
    cancel: async (source) => {
      const receipt = parseDeferredReceiptV1(source);
      if (receipt.ownership.disposition !== "retained") throw new TypeError("invalid deferral transition");
      await port.cancel(receipt);
    },
    resume: async (source, freshProcess) => {
      const receipt = parseDeferredReceiptV1(source), now = nowMs();
      if (receipt.ownership.disposition !== "retained" || now < receipt.resume_after_ms || now > receipt.resume_by_ms) throw new TypeError("invalid deferral transition");
      const state = await port.state(receipt);
      if (state !== "resumable") return { status: "blocked" };
      return freshProcess(receipt.bindings, receipt.fresh_approval_required);
    }
  };
}
