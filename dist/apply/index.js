...
async function bounded4(response, signal) {
  ...
  if (canonical(parsed) !== text4) throw new ApplyCoordinationError("YKP-COORD-001");
  return parsed;
}
...
function calculateEffectiveSchema(policy, observed) {
  ...
  for (const fieldKey of Object.keys(policy.fields).sort()) {
    ...
    const exact2 = observed.fields.filter((field2) => field2.name === desired.name);
...
async function readWithClient(input3, options, client) {
  ...
  const fields = fieldsPage.nodes.filter((f) => ["text", "number", "date", "single_select", "iteration"].includes(String(f.data_type))).map((f) => ({ id: String(integer(f.id)), name: text3(f.name, 128), kind: kind(f.data_type), options: fieldOptions(f) }));
