...
test("release preflight has read-only permissions and no publisher command",async()=>{const workflow=await readFile(".github/workflows/release-preflight.yml","utf8");assert.match(workflow,/permissions:\n  contents: read/u);assert.doesNotMatch(workflow,/contents: write|id-token: write|attestations: write|gh release|npm publish|git tag/u);assert.match(workflow,/publication authority/u);});
