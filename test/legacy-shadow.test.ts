...
test("compatibility matrix declares the bounded controlled-apply delta",()=>{const apply=LEGACY_COMPATIBILITY_MATRIX.find(entry=>entry.capability.startsWith("controlled apply"));assert.equal(apply?.state,"Changed");assert.match(apply?.note??"",/legacy-apply-v1/u);assert.equal(LEGACY_COMPATIBILITY_MATRIX.find(entry=>entry.capability==="Project-owned Status")?.state,"Supported");});
