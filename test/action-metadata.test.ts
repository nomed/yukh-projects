...
test("Action requires read access without claiming a read-only token",async()=>{const source=await readFile("action.yml","utf8");assert.match(source,/description: GitHub credential with the required read access/u);assert.doesNotMatch(source,/description: Read-only GitHub credential/u);});
