...
export function planLegacyReconciliation(policySource:string,snapshot:RestProjectSnapshot,issueNumber:number):ReconciliationPlan{
 ...
 for(const key of Object.keys(policy.fields).sort()){
  ...
  if(declaration.target==="issue_type"){
   if(observed.issueType===desired)continue;const binding=snapshot.issueTypes?.find(value=>value.name===desired);if(!binding)throw new TypeError("legacy issue type binding is unavailable");operations.push({operationKey:operationKey("issue","type","set"),type:"set_issue_type",subject:{ref:scope.subjectRef},resource:{kind:"issue_type",logicalKey:key,scopeRef:scope.repositoryRef,providerRef:binding.id},action:"set",environment:"dry-run",reason:"legacy.issue_type.differs",preconditions:[{kind:"old_value",logicalKey:key,expected:observed.issueType??null}],dependsOn:[],desired});continue;
  }
...
export async function runLegacyDryRun(input:{ownerLogin:string;repositoryName:string;projectNumber:number;issueNumber:number;policySource:string;token:string},reader:typeof readRestProjectSnapshot=readRestProjectSnapshot):Promise<DryRunResult>{try{const policy=parseLegacyPolicy(input.policySource),includeIssueTypes=Object.values(policy.fields).some(field=>field.target==="issue_type"),snapshot=await reader({ownerLogin:input.ownerLogin,repositoryName:input.repositoryName,projectNumber:input.projectNumber,issueNumbers:[input.issueNumber]},{token:input.token,graphqlRemaining:0,includeIssueTypes}),plan=planLegacyReconciliation(input.policySource,snapshot,input.issueNumber);return{status:"success",report:renderPublicReport(plan)};}catch{return{status:"error",failureClass:"invariant",diagnostics:[{code:"YKP-LEGACY-001",message:"dry-run could not produce a complete report"}]};}}
