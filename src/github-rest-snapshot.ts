...
export interface RestProjectSnapshot {
 subjectRef:string;ownerKind:OwnerKind;ownerLogin:string;repositoryName:string;projectNumber:number;repositoryRef:string;projectRef:string;
...
async function readWithClient(input:SnapshotInput,options:RestSnapshotOptions,client:RestSnapshotClient):Promise<RestProjectSnapshot>{
 ...
 return{subjectRef:subject(options.token),ownerKind,ownerLogin:input.ownerLogin,repositoryName:input.repositoryName,projectNumber:input.projectNumber,repositoryRef:text(repo.node_id),projectRef,fields:fields.sort((a,b)=>a.id.localeCompare(b.id)),issues,evidence:{...client.evidence}};
}
