...
export interface RestProjectSnapshot {
 subjectRef:string;ownerKind:OwnerKind;ownerLogin:string;repositoryOwnerKind?:OwnerKind;projectOwnerKind?:OwnerKind;projectOwnerLogin?:string;repositoryName:string;projectNumber:number;repositoryRef:string;projectRef:string;
...
class RestSnapshotClient {
 ...
 invalidate(input:SnapshotInput,effect:SnapshotInvalidationEffect):void{const projectOwnerLogin=input.projectOwnerLogin??input.ownerLogin;if(!/^[A-Za-z0-9-]{1,39}$/u.test(projectOwnerLogin)||!Number.isSafeInteger(input.projectNumber)||input.projectNumber<1)throw new GitHubTransportError("YKP-GH-READ-001");const prefix=new RegExp(`^/(?:orgs|users)/${projectOwnerLogin}/projectsV2/${input.projectNumber}/(?:${effect==="schema"?"fields|items":"items"})\\?`,`u`),keys=new Set([...this.cache.keys(),...this.flights.keys()]);for(const key of keys)if(prefix.test(key)){this.generations.set(key,(this.generations.get(key)??0)+1);this.cache.delete(key);this.flights.delete(key);}}
...
export type SnapshotInput={ownerLogin:string;repositoryName:string;projectOwnerLogin?:string;projectNumber:number;issueNumbers:readonly number[]};
