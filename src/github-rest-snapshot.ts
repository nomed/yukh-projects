...
class RestSnapshotClient {
 private readonly request:typeof globalThis.fetch;private readonly now:()=>number;private readonly ttl:number;private readonly cache=new Map<string,Cached>();private readonly flights=new Map<string,Promise<RestPage>>();
 private restRemaining=Number.POSITIVE_INFINITY;private graphqlRemaining:number;private bytes=0;
 ...
 private updateRate(headers:Headers):void{const value=headers.get("x-ratelimit-remaining");if(value!==null&&/^\d+$/u.test(value))this.restRemaining=Number(value);}
 async get(path:string):Promise<RestPage>{
  ...
  const task=(async()=>{const reserve=this.options.restReserve??500;if(this.restRemaining<=reserve)throw new GitHubTransportError("YKP-RATE-001");if(this.evidence.restRequests>=(this.options.maxRestRequests??64))throw new GitHubTransportError("YKP-RATE-001");this.evidence.restRequests++;if(cached?.etag)this.evidence.conditionalRequests++;let response:Response;try{response=await this.request(`${API}${path}`,{method:"GET",redirect:"manual",headers:this.headers(cached?.etag)});}catch{throw new GitHubTransportError("YKP-GH-READ-004");}this.updateRate(response.headers);if(response.status===304&&cached){const refreshed={...cached,expires:current+this.ttl};this.cache.set(key,refreshed);return{body:refreshed.body,bytes:0,headers:new Headers(refreshed.link?{link:refreshed.link}:{})};}if(response.status>=300&&response.status<400||!response.ok)this.classify(response);if(!response.headers.get("content-type")?.toLowerCase().includes("json"))throw new GitHubTransportError("YKP-REST-001");const raw=new Uint8Array(await response.arrayBuffer());this.bytes+=raw.byteLength;if(raw.byteLength>8*1024*1024||this.bytes>64*1024*1024)throw new GitHubTransportError("YKP-GH-READ-005");let body:unknown;try{body=JSON.parse(new TextDecoder("utf-8",{fatal:true}).decode(raw));}catch{throw new GitHubTransportError("YKP-REST-001");}this.cache.set(key,{body,bytes:raw.byteLength,etag:response.headers.get("etag")??undefined,link:response.headers.get("link")??undefined,expires:current+this.ttl});return{body,bytes:raw.byteLength,headers:response.headers};})();
 ...
 async relationships(ids:readonly string[]):Promise<Map<string,{number:number;parent?:number;blockedBy:number[];blocking:number[]}>>{const result=new Map<string,{number:number;parent?:number;blockedBy:number[];blocking:number[]}>();if(ids.length===0)return result;if(ids.length>100)throw new GitHubTransportError("YKP-GH-READ-005");if(this.graphqlRemaining<=0)return result;const reserve=this.options.graphqlReserve??500;if(this.graphqlRemaining<=reserve)throw new GitHubTransportError("YKP-RATE-001");if(this.evidence.graphqlRequests>=(this.options.maxGraphqlRequests??2))throw new GitHubTransportError("YKP-RATE-001");this.evidence.graphqlRequests++;let response:Response;try{response=await this.request(GRAPHQL,{method:"POST",redirect:"manual",headers:{accept:"application/vnd.github+json","content-type":"application/json",authorization:`Bearer ${this.options.token}`,"x-github-api-version":"2022-11-28"},body:JSON.stringify({query:RELATIONSHIP_QUERY,variables:{ids}})});}catch{throw new GitHubTransportError("YKP-GH-READ-004");}if(!response.ok)this.classify(response);let payload:unknown;try{payload=await response.json();}catch{throw new GitHubTransportError("YKP-REST-001");}if(!rec(payload)||Array.isArray(payload.errors)||!rec(payload.data)||!Array.isArray(payload.data.nodes)||!rec(payload.data.rateLimit))throw new GitHubTransportError("YKP-REST-001");this.graphqlRemaining=Number(payload.data.rateLimit.remaining);for(const node of payload.data.nodes){if(!rec(node))throw new GitHubTransportError("YKP-REST-001");const connections=["subIssues","blockedBy","blocking"].map(name=>{const c=node[name];if(!rec(c)||!Array.isArray(c.nodes)||!rec(c.pageInfo)||c.pageInfo.hasNextPage===true)throw new GitHubTransportError("YKP-GH-READ-005");return c.nodes.map(v=>{if(!rec(v))throw new GitHubTransportError("YKP-REST-001");return integer(v.number);});});result.set(text(node.id),{number:integer(node.number),...(rec(node.parent)?{parent:integer(node.parent.number)}:{}),blockedBy:connections[1]!,blocking:connections[2]!});}return result;}
}
...
export function createRestProjectSnapshotReader(options:RestSnapshotOptions):{read(input:SnapshotInput):Promise<RestProjectSnapshot>}{const client=new RestSnapshotClient(options);return{read:input=>readWithClient(input,options,client)};}
