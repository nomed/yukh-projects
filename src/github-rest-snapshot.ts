...
async function readWithClient(input:SnapshotInput,options:RestSnapshotOptions,client:RestSnapshotClient):Promise<RestProjectSnapshot>{
 ...
 const issueTypes=options.includeIssueTypes?(await client.list(`/repos/${input.ownerLogin}/${input.repositoryName}/issue-types?per_page=100`)).nodes.map(value=>({id:text(value.node_id),name:text(value.name,128)})).sort((a,b)=>a.id.localeCompare(b.id)):undefined;
