...
export interface GitHubRateLedgerSnapshot{restRequests:number;graphqlRequests:number;graphqlPoints:number;restRemaining:number;graphqlRemaining:number}
...
export function createGitHubRateLedger(options:GitHubRateLedgerOptions={}):GitHubRateLedger{
 ...
 return{
  reserve:(resource,cost=1)=>{if(!finiteNonnegative(cost)||cost<1)return false;if(resource==="rest"){if(restRequests>=maxRestRequests||restRemaining-cost<restReserve)return false;restRequests++;if(Number.isFinite(restRemaining))restRemaining-=cost;return true;}if(resource!=="graphql"||graphqlRequests>=maxGraphqlRequests||graphqlPoints+cost>maxGraphqlPoints||graphqlRemaining-cost<graphqlReserve)return false;graphqlRequests++;graphqlPoints+=cost;if(Number.isFinite(graphqlRemaining))graphqlRemaining-=cost;return true;},
