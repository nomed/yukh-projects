...
import { readGitHubObservation, type GitHubObservation, type ReadOnlyTransport, type RequestedGitHubScope } from "./github-readonly.js";
import { calculateEffectiveSchema, parseRepositoryPolicy, type ObservedSchema } from "./policy.js";
import { planReconciliation, renderPublicReport, type ObservedItem, type PublicReport, type ReconciliationPlan } from "./planner.js";
import type { EffectiveSchemaResult, RepositoryPolicy } from "./policy.js";
...
export type PreparedReconciliation={status:"success";plan:ReconciliationPlan;observation:GitHubObservation;policy:RepositoryPolicy;schema:EffectiveSchemaResult}|DryRunFailure;
...
export async function prepareReconciliation(input:DryRunInput):Promise<PreparedReconciliation>{
...
export async function runDryRun(input:DryRunInput):Promise<DryRunResult>{
 const prepared=await prepareReconciliation(input);return prepared.status==="success"?{status:"success",report:renderPublicReport(prepared.plan)}:prepared;
}
